...
action: Install the rebuilt APK on `<TARGET_DEVICE_IP>:5555`, re-check the failing WebDAV/local folder thumbnail scenes and the remote/local video player immersive scenes, then save screenshots/UI dumps/logcat/gfxinfo evidence under the dated artifacts directory.
loop: false
verify:
  type: artifact
