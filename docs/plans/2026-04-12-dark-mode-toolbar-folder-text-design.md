...
- Real-device ADB verification on `<TARGET_DEVICE_IP>:5555` captures screenshot/UI/activity evidence for the affected screens
risk_level: low
...
- confirm: ADB evidence on `<TARGET_DEVICE_IP>:5555` shows the settings page and folder list remain readable in dark mode
