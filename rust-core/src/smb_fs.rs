...
use smb::{
    Client, ClientConfig, ConnectionConfig, Dialect, Directory, Error as SmbError, FileAccessMask,
    FileCreateArgs, FileDispositionInformation, FileFullDirectoryInformation, GetLen, Resource,
    ResourceHandle, Status, UncPath,
};
...
/// Protocol value for `ShareKind::Disk` in the SMB SRVSVC response.
const DISK_SHARE_KIND: u8 = 0;

/// SMB Android compatibility profile shared by browsing and share discovery.
///
...
fn qualify_username(username: &str, domain: Option<&str>) -> String {
    match domain {
...
fn map_disk_share(share_kind: u8, name: String, remark: String) -> Option<SmbShare> {
    if share_kind != DISK_SHARE_KIND || name.ends_with('$') {
        return None;
    }

    Some(SmbShare { name, remark })
}
            ...
            shares
                ...
                .filter_map(|share| {
                    let name = share
                        .netname
                        .as_ref()
                        .map(|value| value.value.to_string().trim_end_matches('\0').to_string())
                        .unwrap_or_default();
                    let remark = share
                        .remark
                        .as_ref()
                        .map(|value| value.value.to_string().trim_end_matches('\0').to_string())
                        .unwrap_or_default();
                    map_disk_share(share.share_type.kind() as u8, name, remark)
                })
                .collect()
        })
        .map_err(|error| classify_srvsvc_error(host, error))
}
...
mod tests {
    ...
    fn qualify_username_adds_domain_only_when_present() {
        ...
        assert_eq!(
            qualify_username("alice", Some("WORKGROUP")),
            r"WORKGROUP\alice"
        );
    }
    ...
    fn map_disk_share_keeps_normal_disk_shares() {
        let share = map_disk_share(DISK_SHARE_KIND, "Media".into(), "Main library".into())
            .expect("disk share should be kept");

        assert_eq!(share.name, "Media");
        assert_eq!(share.remark, "Main library");
    }

    #[test]
    fn map_disk_share_drops_admin_shares() {
        assert!(map_disk_share(DISK_SHARE_KIND, "C$".into(), "Default share".into()).is_none());
    }

    #[test]
    fn map_disk_share_drops_non_disk_shares() {
        assert!(map_disk_share(3, "IPC$".into(), "Remote IPC".into()).is_none());
    }
}
