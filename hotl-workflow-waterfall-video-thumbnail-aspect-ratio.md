...
action: Install the latest debug build on `<TARGET_DEVICE_IP>:5555`, open a waterfall media page with portrait videos, and collect screenshot/log/activity evidence showing video card heights now follow the thumbnail aspect ratio instead of a forced uniform crop.
loop: false
verify:
  - type: artifact
